...
def projection_mirror():
    """A second-tab view of the calibrated output, rendered client-side by
    the same module as the admin panel's preview lightbox -- see
    templates/mirror.html. It used to poll a JPEG of the projector's real
    framebuffer, which cost the Pi a full-screen glReadPixels every 0.3s
    whether or not anyone was watching (~16% of its frame rate) plus an
    ffmpeg encode per poll; drawing it on the viewing device instead costs
    the Pi only the mapping poll. Not gated behind admin mode: it shows
    nothing beyond the model and its calibration, so there's nothing
